...
def validate_is_map(img_bgr: np.ndarray) -> tuple[bool, str]:
    ...
    bright_isolated = float(np.sum((V > 200) & (S < 30)) / total_px)
    green = float(np.sum((H > 25) & (H < 90) & (S > 55) & (V > 50)) / total_px)
    logger.info("Map validation: bright_isolated=%.3f green_terrain=%.3f", bright_isolated, green)
    if bright_isolated > 0.05 and green < 0.03:
        return False, "This looks like a gameplay screenshot. Please open the map in Forza (Menu → Map) and screenshot that instead."
    return True, ""
